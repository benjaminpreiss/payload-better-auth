 ...
 */ async function extractAllSessionTokens(headers, storage) {
 ...
 */ async function validateSessionToken(fullToken, storage) {
 ...
 */ function createBetterAuthStrategy(storage, _prefix) {
    return {
        name: 'better-auth',
 ...
 */ function createComposableAccess(storage, developerAccess, operation) {
 ...
 */ export function extendUsersCollection(baseCollection, options) {
    ...
    const baBeforeChange = async ({ data, operation, originalDoc, req })=>{
        const sig = req.context.baSig;
        if (operation === 'create' && sig) {
            const expectedBody = {
                collection: 'users',
                op: 'create',
                userId: data.baUserId
            };
    ...
    const baBeforeDelete = async ({ req })=>{
    ...
    return {
        ...
        access: {
            ...
            create: createComposableAccess(storage, developerAccess.create, 'create'),
            delete: createComposableAccess(storage, developerAccess.delete, 'delete'),
            read: createComposableAccess(storage, developerAccess.read, 'read'),
            update: createComposableAccess(storage, developerAccess.update, 'update')
        },
        admin: {
            ...base.admin,
            defaultColumns: (()=>{
                const cols = base.admin?.defaultColumns ?? [
                    'email',
                    'createdAt'
                ];
 ...
 */ export function createMinimalUsersCollection(options) {
